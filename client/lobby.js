...
class Lobby {
    ...
    checkWin(socket) {
        if(socket.cards.length == 0) {
            this.io.emit('chatReceive', {text:(socket.playerID + ' won the round!')});
            this.roundEnd();
            setTimeout(this.newRound.bind(this), 3000);
            this.io.emit('win', {playerID:socket.playerID, seat:socket.seat});
            return true;
        }
    }
    ...
    uno(socket) {
        ...
        socket.saidUno = true;
        this.io.emit('uno', {seat:socket.seat});
    }
